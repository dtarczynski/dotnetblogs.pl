...
(function() {
 ...
 var cornerToolTip = 'topLeft';
        var cornerTarget = 'bottomRight';

        function partialDone(feedData) {
            //doSort();
            $("#headlines").setTemplateElement("allFeeds");
            $("#headlines").processTemplate(data);
        }

        function getProperDate(wrongDate) {
            var date = new Date(wrongDate);

            return date.toLocaleDateString() + ' | ' + date.toLocaleTimeString();
        }

        function changeToNew(wrongDate) {
            var date = new Date(wrongDate);
            var minDate = Date.today().addDays(-7);
            if (date > minDate) {
                return 'class="newPostLink"';
            }
        }

        $.fn.qtip.styles.codaStyle = { // Last part is the name of the style
            background: '#464360', // 1e1b2d
            border: {
                color: '#1e1b2d', //  13111c
                width: 2,
                radius: 4
        ...
        function sortByDate(first, last) {
            ...
            var firstDate = new Date(first.entries[0].publishedDate);
            var lastDate = new Date(last.entries[0].publishedDate);
        ...
        function sortByTitle(first, last) {
            //alert(first.engTitle);
            if (first.engTitle < last.engTitle) return -1;
            if (first.engTitle > last.engTitle) return 1;
            return 0;
        }
        ...
        function convert_plToEng(val) {
            ...
            for (i = 0; i < val.length; i++) {
                switch (val.charAt(i)) {
                    ...
                    case "æ": str.append("c"); break;
                    case "ê": str.append("e"); break;
                    case "³": str.append("l"); break;
                    case "ñ": str.append("n"); break;
                    case "ó": str.append("o"); break;
                    case "œ": str.append("s"); break;
                    case "Ÿ": str.append("z"); break;
                    ...
                    case "": str.append("Z"); break;
                    case "¯": str.append("Z"); break;
                    default: str.append(val.charAt(i)); break;
                }
            }
        ...
        function StringBuilder(val) {
            ...
            this.length = StringBuilder_Length;
        ...
        function StringBuilder_Append(val) {
            if (val == null) {
                return;
            }
            this.bufferLength += val.length;
